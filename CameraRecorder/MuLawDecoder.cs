namespace NAudio.Codecs
{
    ...
    public static class MuLawDecoder
    {
        ...
        {
            // Каждый байт mu-law (8 бит) превращается в 16-битный PCM сэмпл
            int sampleCount = muLawData.Length;
            byte[] pcmData = new byte[sampleCount * 2]; // 16-bit = 2 байта на сэмпл

            for (int i = 0; i < sampleCount; i++)
            {
                // Декодируем один mu-law сэмпл в 16-битный PCM
                short pcmSample = MuLawToLinearSample(muLawData[i]);

                // Конвертируем short в два байта (little-endian)
                pcmData[i * 2] = (byte)(pcmSample & 0xFF);       // Младший байт
                pcmData[i * 2 + 1] = (byte)((pcmSample >> 8) & 0xFF); // Старший байт
            }
