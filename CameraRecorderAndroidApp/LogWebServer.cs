...
public class LogWebServer
{
    ...
    public LogWebServer(IOptions<CameraRecorderSettings> options,
        [FromKeyedServices("LogsDirectory")] string logsDir,
        ILogger<LogWebServer> logger)
    {
        var port = "8080";
        _recDir = options.Value.LocalStorage.Path;
        ...
        {
            _recDir = Path.Combine(Android.OS.Environment.ExternalStorageDirectory.AbsolutePath, _recDir);
        }
    ...
    private async Task HandleRequest(HttpListenerContext ctx)
    {
        ...
        catch (HttpListenerException ex) when (ex.ErrorCode == -2146232800)
        {
